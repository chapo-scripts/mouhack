...
return function(page)
    page.config.animSpeed = imgui.new.bool(false)
    page.config.animSpeedMultiplier = imgui.new.float(2)

    page:On("loop", function()
       if (page.config.animSpeed[0]) then
            for _, anim in ipairs(animationsList) do
                setCharAnimSpeed(PLAYER_PED, anim, page.config.animSpeedMultiplier[0])
            end
        end
    end)

    return Funcs:new(FuncType.Toggle, {
        label = "Изменять скорость анимаций",
        unsafe = true,
        value = page.config.animSpeed,
        options = {
            Funcs:new(FuncType.SliderFloat, {
                label = "Множитель скорости",
                value = page.config.animSpeedMultiplier,
                min = 0.1,
                max = 10
            })
        }
